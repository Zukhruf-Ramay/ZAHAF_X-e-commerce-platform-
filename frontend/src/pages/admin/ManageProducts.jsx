...
const ManageProducts = () => {
  ...
  const updateImageUrl = (i, v) => setForm(p => {
    const imgs = [...p.images]; imgs[i] = v; return { ...p, images: imgs }
  })

  const addFeatureField = () => setForm(p => ({ ...p, features: [...p.features, ''] }))
  const removeFeatureField = (i) => setForm(p => ({ ...p, features: p.features.filter((_, idx) => idx !== i) }))
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold mb-4">
              <i className="fas fa-edit mr-2 text-blue-500"></i> {editing ? 'Edit Product' : 'Add Product'}
            </h2>
